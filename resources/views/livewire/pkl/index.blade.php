                            ...
                            <tr class="hover:bg-blue-50 transition-colors duration-150">
                                ...
                                <td class="px-6 py-3 text-center whitespace-nowrap">
                                    <div x-data="{ open: false }" class="relative inline-block">
                                        ...
                                        {{-- Role Siswa dan Super Admin - Semua tombol dropdown --}}
                                        @elseif(auth()->user()->hasRole(['Siswa', 'super_admin']))
                                            <div x-data="{ open: false }" class="relative inline-block">
                                                ...
                                                <div x-show="open" x-transition @click.away="open = false"
                                                    ...
                                                    <a href="{{ route('pkl.show', ['id' => $pkl->id]) }}"
                                                    class="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-blue-50 hover:text-blue-700 rounded-t-lg">
                                                    ...
                                                    @if(auth()->user()->hasRole(['Siswa', 'super_admin']))
                                                        <button
                                                            @click.prevent="
                                                                if (confirm('Yakin ingin menghapus data PKL ini?')) {
                                                                    $wire.delete({{ $pkl->id }});
                                                                    open = false;
                                                                }
                                                            "
                                                            class="flex items-center w-full px-4 py-2 text-sm text-red-600 hover:bg-red-50 hover:text-red-700 rounded-b-lg focus:outline-none"
                                                        >
                                                            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
